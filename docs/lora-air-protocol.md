...
SW6106 PMIC on the ESP32's side of the node's internal I²C link, so this field is
filled from `TB_REG_HOST_BATTERY` — 0xFF for the first seconds after boot and
whenever that read fails, a real percentage otherwise.
...
5. `LORA_POLL_CMD_REPORT` → snapshot, fill `lora_vital_t`, `node_id` = own id,
   ...
   the gauge or 0xFF if there is no reading.
